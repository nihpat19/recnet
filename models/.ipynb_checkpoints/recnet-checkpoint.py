...
class RecNetBlock_SharedBN(nn.Module):

    def __init__(self, in_channels, out_channels, stride=1, downsample=None, parent=None):
        super(RecNetBlock_SharedBN, self).__init__()
        self.parent=parent
        if parent is None:
            self.convs=nn.ModuleList([conv3x3(in_channels, out_channels, stride), conv3x3(out_channels, out_channels, stride=1)])
            self.batchNorms=nn.ModuleList([nn.BatchNorm2d(out_channels) for _ in range(2)])
            ...
            self.stride= parent.stride
            self.downsample=parent.downsample
            self.batchNorms=parent.batchNorms
        self.relu=nn.ReLU(inplace=True)
...
class RecNet_SharedBNs(nn.Module):
    def __init__(self, block, layers, num_classes=100):
        super(RecNet_SharedBNs, self).__init__()
        self.inplanes = 16
        self.conv1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(16)
        self.sizes = [16, 32, 64]
...
def recnet_sharedbn(**kwargs):
    model = RecNet_SharedBNs(RecNetBlock_SharedBN, [9,9,9], **kwargs)
    return model
